...
// Binary extensions worth scanning for in assistant message text.
// Text files (js, py, etc.) are always written via file-write tools
// with content in args, so they're caught by tool-arg detection.
// Binary files are often generated by running scripts, so we need
// message-scanning to detect them.
